#' @title Create waiting time variable
#'
#' @description
#' Creates a "waiting time" variable by computing the difference between the date the order was created, and the date it was delivered.
#'
#' @param df A dataframe of Delivraide deliveries.
#' @param creat_date The name of the column representing the order creation date.
#' @param deliv_date The name of the column representing the order delivery date.
#'
#' @return The input dataframe with an additional column "waiting_days" representing the waiting time in days.
#'
...
create_waiting_days <- function(df, creat_date = createdAt, deliv_date = updatedAt){
  ...
  stopifnot("Creation and delivery columns must be in a date format" = is.instant(df[[substitute(creat_date)]]) && is.instant(df[[substitute(deliv_date)]]))
